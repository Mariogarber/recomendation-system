...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--artifact-root",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "artifacts" / "known_user_deep_router_v1",
    )
    parser.add_argument("--save-path", type=Path, default=None)
    parser.add_argument(
        "--incumbent-router-root",
        type=Path,
        default=None,
        help="Override the incumbent router root stored in router_spec.joblib. "
             "Use this to plug in a retrained cold model without regenerating the deep artifact.",
    )
...
def main() -> None:
    ...
    incumbent_root = (
        Path(args.incumbent_router_root)
        if args.incumbent_router_root is not None
        else Path(router_spec["incumbent_router_root"])
    )
    ...
    # NOTE: user_average_stars NOT overridden here — must match the features the incumbent was trained with.
    # See comment in train_known_user_deep.py.
