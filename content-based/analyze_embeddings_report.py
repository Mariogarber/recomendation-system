...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--artifacts-root",
        type=Path,
        default=Path(__file__).resolve().parent / "artifacts" / "competition_embeddings_v1",
    )
    parser.add_argument("--data-dir", type=Path, default=get_default_data_dir())
    parser.add_argument(
        "--report-dir",
        type=Path,
        default=Path(__file__).resolve().parent / "artifacts" / "competition_embeddings_v1" / "report",
    )
...
def load_artifacts(root: Path) -> LoadedArtifacts:
    ...
    return LoadedArtifacts(
        business_ids=pd.read_csv(business_dir / "business_ids.csv")["business_id"],
        business_full=load_sparse_npz(business_dir / "business_full_features.npz"),
        business_deep=load_dense_npz(deep_dir / "business_deep_features.npz"),
        business_table=pd.read_csv(business_dir / "clean_business_table.csv", low_memory=False),
        business_summary=load_json(business_dir / "business_representation_summary.json"),
        business_block_summary=pd.read_csv(business_dir / "business_block_summary.csv"),
        user_manual_ids=pd.read_csv(manual_dir / "user_ids.csv")["user_id"],
        user_manual_profile=load_sparse_npz(manual_dir / "user_profile_features.npz"),
        user_manual_table=pd.read_csv(manual_dir / "clean_user_table.csv", low_memory=False),
        user_manual_summary=load_json(manual_dir / "user_profile_summary.json"),
        user_deep_ids=pd.read_csv(deep_dir / "user_deep_ids.csv")["user_id"],
        user_deep=load_dense_npz(deep_dir / "user_deep_features.npz"),
        user_deep_table=pd.read_csv(deep_dir / "user_deep_clean_table.csv", low_memory=False),
        user_deep_summary=load_json(deep_dir / "user_deep_summary.json"),
    )
...
def build_utility_metrics(artifacts: LoadedArtifacts, train_reviews: pd.DataFrame) -> dict[str, Any]:
    ...
    utility_rows = [
        {"space": "manual_profile + business_full", "evaluation": "diagnostico post-export con scorer lineal ligero", "mae": float(mean_absolute_error(y_val, manual_pred)), "rmse": rmse(y_val, manual_pred)},
        {"space": "user_deep + business_deep", "evaluation": "diagnostico post-export con scorer lineal ligero", "mae": float(mean_absolute_error(y_val, deep_pred)), "rmse": rmse(y_val, deep_pred)},
        {"space": "deep_user_encoder original", "evaluation": "validacion temporal del entrenamiento", "mae": float(artifacts.user_deep_summary["best_val_mae"]), "rmse": float(artifacts.user_deep_summary["best_val_rmse"])},
    ]
    ...
    for history_band in ["0", "1", "2-5", "6-20", ">20"]:
        ...
        if int(mask.sum()) == 0:
            continue
        band_rows.append({"history_band": history_band, "manual_mae": float(mean_absolute_error(y_val[mask], manual_pred[mask])), "deep_mae": float(mean_absolute_error(y_val[mask], deep_pred[mask])), "n_samples": int(mask.sum())})
    return {
        "summary_table": pd.DataFrame(utility_rows),
...
def main() -> None:
    ...
    utility["summary_html"] = styled_table(utility["summary_table"], float_cols=["mae", "rmse"])
    ...
    utility["section_intro_html"] = section_intro_html(
        "Este bloque contesta la pregunta importante: sirven realmente los embeddings para predecir ratings y ordenar preferencias?"
    )
    utility["summary_note_html"] = explainer_html(
        measures="Compara MAE y RMSE entre espacios de scoring.",
        read="Menor error significa mejor utilidad para la tarea final; aqui manda la comparacion entre manual, deep y el baseline temporal.",
        good_bad="Bueno: el espacio deep reduce MAE/RMSE frente al manual. Malo: mejora nula o peor que el baseline.",
        warning="Un buen MAE global puede ocultar problemas graves en cold-start; baja siempre a las bandas de historial.",
    )
    ...
    html_report = build_report_html(coverage=coverage, utility=utility, business_metrics=business_metrics, user_consistency=user_consistency, clusters=clusters, friends=friends, recommendations=recommendations)
    save_outputs(args.report_dir, coverage=coverage, utility=utility, business_metrics=business_metrics, clusters=clusters, friends=friends, html_report=html_report)
